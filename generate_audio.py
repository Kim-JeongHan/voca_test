...
def filter_existing_audio(words: Set[str], audio_dir: Path) -> Tuple[Set[str], Set[str]]:
    """
...
def fetch_from_elevenlabs(word: str, api_key: str) -> bytes | None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  ⚠️  ElevenLabs API error for '{word}': HTTP {response.status_code}")
...
def generate_audio_for_word(word: str, output_path: Path, use_elevenlabs: bool = True) -> bool:
    """
...
def main():
    ...
    words_to_generate, existing_words = filter_existing_audio(all_words, AUDIO_OUTPUT_DIR)
    ...
    if use_elevenlabs:
        ...
        print(f"   ⚠️  ElevenLabs API key: NOT configured (will use Dictionary API only)")
    ...
    print(f"   📁 Total files in {AUDIO_OUTPUT_DIR.name}/: {len(list(AUDIO_OUTPUT_DIR.glob('*.mp3')))}")
